...
public class SpeedDataUI extends FlightDataUI{
    ...
    public void writeValues() {
        ...
        font.draw(batch, "IAS: " + formatOneDecimal(UnitConversionUtils.convertMps2Knts(wing.getIndicatedAirspeed())) + " knts", x + 2, y);
        font.draw(batch, "GS: " + formatOneDecimal(UnitConversionUtils.convertMps2Knts(aircraft.getGroundSpeed())) + " knts", x + 2, y - 20);
        font.draw(batch, "Headwind: " + formatOneDecimal(UnitConversionUtils.convertMps2Knts(-wing.getWind().getVelocity().x)) + " knts", x + 2, y - 40);
